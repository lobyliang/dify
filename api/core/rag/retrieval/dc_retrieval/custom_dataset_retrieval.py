...
class CustomDataSetRetrieval:
    ...
    def get_chuncks_by_order(self, doc_id, page_size, page_number):
        ...
        docs = (
            ...
            .order_by(DocumentSegment.hit_count.desc())
    ...
    def get_datasets(sef, tanent_id):
        ...
        try:
            datasets = (
                db.session.query(Dataset).filter(Dataset.tenant_id == tanent_id).all()
            )
            for ds in datasets:
                item = {}
                ds_docs = (
                    ...
                    .filter(
                        DatasetDocument.dataset_id == ds.id
                        and DatasetDocument.indexing_status == "completed"
                        and DatasetDocument.enabled == True
                    )
                    .all()
                )
    ...
    def reorgenazie_output(
        ...
        if model_config.mode == ModelMode.CHAT.value:
            system_prompt_messages = ChatModelMessage(
                role=PromptMessageRole.SYSTEM, text=system_prompt
            )
            prompt_messages.append(system_prompt_messages)
            user_prompt_message = ChatModelMessage(
                role=PromptMessageRole.USER, text=user_prompt
            )
    ...
    def retrieval(
        ...
        if not CustomDataSetRetrieval.reranking_model_name:
            default_rerank = (
                CustomDataSetRetrieval.model_manager.get_default_model_instance(
                    tenant_id=tenant_id, model_type=ModelType.RERANK
                )
            )
            if default_rerank:
        ...
        if retrieve_strategy == DatasetRetrieveConfigEntity.RetrieveStrategy.SINGLE:
            all_documents, query_id = self.single_retrieve(
                app_id,
                tenant_id,
                user_id,
                user_from,
            ...
            all_documents, query_id = self.multiple_retrieve(
                app_id,
                tenant_id,
                user_id,
                user_from,
        ...
        if segments:
            ...
            for segment in sorted_segments:
                ...
                doc_res["attach_files"] = FileInfoService.get_chunck_attach_files_info(
                    segment.id
                )
    ...
    def single_retrieve(
        ...
        if dataset_id:
            ...
            if dataset:
                ...
                endUser = CustomDataSetRetrieval.get_end_user(
                    user_id=user_id, tenant_id=tenant_id, app_id=app_id
                )

                query_id = self._on_query(
                    query, [dataset_id], app_id, user_from, endUser.id
                )
                if results:
    ...
    def comment_rag_query(query_id: str, seg_id: str, rate: float, like: str):
        ...
        rag_query = (
            db.session.query(DatasetQuery).filter(DatasetQuery.id == query_id).first()
        )
        if not rag_query:
    ...
    def dataset_comment_list(dataset_id: str, page_no: int, page_size: int):
        ...
        query = (
            ...
            .order_by(
                likes_summary.c.total_likes.desc(), likes_summary.c.last_date.desc()
            )
            .offset((page_no - 1) * page_size)
            .limit(page_size)
        )
        total_count = (
            db.session.query(DatasetQuery.content)
            .filter(DatasetQuery.dataset_id == dataset_id)
            .group_by(DatasetQuery.content)
            .count()
        )
...
"""
    ...
    @staticmethod
    def chunck_comment_list(
        segment_id: str, page_no: int = None, page_size: int = None
    ):
        query = (
