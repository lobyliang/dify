...
class OpenAIText2SpeechModel(_CommonAli, TTSModel):
    """
    ...
    def _invoke(self, model: str, tenant_id: str, credentials: dict,
        ...
        :param model: 模型名称
        :param tenant_id: 用户持有它
        :param credentials: 凭证
        :param content_text: 文本文件
        :param voice: 说话人
        :param streaming:输出是流
        :param user: 仅用户
    ...
    def _tts_invoke_streaming(self, model: str, tenant_id: str, credentials: dict, content_text: str,
        ...
        try:
            ...
            for sentence in sentences:
                ...
                response = post("https://nls-gateway-cn-shanghai.aliyuncs.com/stream/v1/tts", params=query_param)
                print("执行阿里云 Content-Type:" + response.headers.get('Content-Type'))
                if response.headers.get('Content-Type') == "application/json":
                    # 出错
                    error_json = response.json()
                    raise InvokeBadRequestError(
                        "status：" + str(error_json["status"]) + "，error:" + error_json["message"])
    ...
    def _process_sentence(self, sentence: str, model: str,
        ...
        if response.headers.get('Content-Type') == "application/json":
            # 出错
            error_json = response.json()
            raise InvokeBadRequestError("status：" + str(error_json["status"]) + "，error:" + error_json["message"])
        else:
