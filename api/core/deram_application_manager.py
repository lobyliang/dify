...
from core.app_runner.deram_assistant_app_runner import DeramAssistantApplicationRunner
...
class DeramApplicationManager:
    ...
    def generate(self, tenant_id: str,
        ...
        """
        # init task id
        print("进入=>>",self.__module__,"ApplicationManager::generate")
        task_id = str(uuid.uuid4())

        # init application generate entity
        print("构造 ApplicationGenerateEntity 对象")
        ...
        (
            conversation,
            message
        ) = self._init_generate_records(application_generate_entity,cmd)

        # init queue manager
        print("构造 ApplicationQueueManager 对象:执行请求")
        ...
        # new thread
        print("构造 worker_thread 对象:开启任务")
        ...
        print("退出",self.__module__,"ApplicationManager::generate"," 返回：_handle_response")
    ...
    def _generate_worker(self, flask_app: Flask,
        ...
        :return:
        """
        print("进入=>>",self.__module__,"ApplicationManager::_generate_worker"," 执行线程内容：启动新的Flask上下文")
        with flask_app.app_context():
            try:
                ...
                if application_generate_entity.app_orchestration_config_entity.agent:
                    # agent app
                    print("创建AssistantApplicationRunner对象，执行run")
                    runner = DeramAssistantApplicationRunner()
                    runner.run(
                        application_generate_entity=application_generate_entity,
                        queue_manager=queue_manager,
                        conversation=conversation,
                ...
                db.session.close()
        print("退出<<=",self.__module__,"ApplicationManager::_generate_worker"," 执行完毕：退出Flask上下文")
    ...
    def _handle_response(self, application_generate_entity: ApplicationGenerateEntity,
        ...
        :return:
        """
        print("进入=>>",self.__module__,"ApplicationManager::_handle_response "," 处理返回，调用GenerateTaskPipeline：process")
        ...
        try:
            print("退出<<=",self.__module__,"ApplicationManager::_handle_response "," 返回，调用GenerateTaskPipeline：process")
    ...
    def _init_generate_records(self, application_generate_entity: ApplicationGenerateEntity,cmd:str) \
        ...
        if not application_generate_entity.conversation_id:
            ...
            conversation = (
                ...
                .filter(
                    ...
                    # Conversation.app_id == app_record.id 梁波改，影响未知
                ).first()
            )
