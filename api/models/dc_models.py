...
class AppCategory(db.Model):
    ...
    __table_args__ = (
        db.PrimaryKeyConstraint('id',name='app_category_pkey'),
    )
    id = db.Column(UUID, server_default=db.text('uuid_generate_v4()'))
...
class DocumentSegmentsAttach(db.Model):
    ...
    attach_type=db.Column(db.String(10),default=db.text('file'))# cover 封面,attach附件
...
class DocumentPrompt(db.Model):
    ...
    __table_args__ = (
        db.PrimaryKeyConstraint('id', name='document_prompt_pkey'),
    )
    id = db.Column(StringUUID, nullable=False,
                   server_default=db.text('uuid_generate_v4()'))
    ...
    updated_at = db.Column(db.DateTime, nullable=True, server_default=db.text('CURRENT_TIMESTAMP(0)'))
