...
class FileInfoService:
    ...
    def get_url_file_info(url):
        try:
            ...
            if content_disposition:
                dispositions = dict(
                    item.split("=") for item in content_disposition.split(";")
                )
    ...
    # 上传文件，并绑定到知识库的分片上
    @staticmethod
    def upload_chunck_attach(
        file: FileStorage, url: str, user: str, doc_seg_id: uuid, isCover: bool = False
    ) -> DocumentSegmentsAttach:
        ...
        if file:
            ...
            if extension.lower() in IMAGE_EXTENSIONS:
                file_size_limit = (
                    current_app.config.get("UPLOAD_IMAGE_FILE_SIZE_LIMIT") * 1024 * 1024
                )
            else:
                file_size_limit = (
                    current_app.config.get("UPLOAD_FILE_SIZE_LIMIT") * 1024 * 1024
                )
    ...
    def attach_file_used(file_uuid: uuid):
        doc_seg_attch = (
            db.session.query(DocumentSegmentsAttach)
            .filter(DocumentSegmentsAttach.file == str(file_uuid))
            .first()
        )
    ...
    # 上传文件，并绑定到知识库的分片上
    @staticmethod
    def bind_attach_to_chunck(
        file_uuid: uuid, url: str, user: str, doc_seg_id: uuid, isCover: bool = False
    ) -> DocumentSegmentsAttach:
        doc_attach = (
            db.session.query(DocumentSegmentsAttach)
        ...
        if file_uuid:
            ...
            upload_file = (
                db.session.query(UploadFile).filter(UploadFile.id == file_uuid).first()
            )
            extension = upload_file.extension
