...
from controllers.service_api.dataset.error import  DocumentAlreadyFinishedError, InvalidActionError
...
class RAGDocumentBatchIndexingEstimateApi(DocumentResource):
    ...
    def get(self,tenant_id, dataset_id, batch):
        ...
        for document in documents:
            ...
            indexing_runner = IndexingRunner()
            try:
                response = indexing_runner.indexing_estimate(tenant_id, extract_settings,# current_user.current_tenant_id
        ...
        if document.data_source_type == 'upload_file':
            ...
            if data_source_info and 'upload_file_id' in data_source_info:
                ...
                try:
                    ...
                    response = indexing_runner.indexing_estimate(tenant_id, [extract_setting],
                                                                 data_process_rule_dict, document.doc_form,
                                                                 'Chinese', dataset_id)
                except LLMBadRequestError:
                    raise ProviderNotInitializeError(
...
#-----------已使用--------------
api.add_resource(RAGDocumentBatchIndexingEstimateApi,
                 '/rag/datasets/<uuid:dataset_id>/batch/<string:batch>/indexing-estimate')
#-----------已使用--------------
api.add_resource(RAGDocumentBatchIndexingStatusApi,
...
#-----------已使用--------------
api.add_resource(RAGDocumentIndexingStatusApi,
                 '/rag/datasets/<uuid:dataset_id>/documents/<uuid:document_id>/indexing-status')
#-----------已使用--------------
api.add_resource(RAGDocumentIndexingEstimateApi,
                 '/rag/datasets/<uuid:dataset_id>/documents/<uuid:document_id>/indexing-estimate')
#############################################
#-----------已使用--------------
api.add_resource(AddDocumentToDatasetApi,
                 '/rag/datasets/<uuid:dataset_id>/documents')

#-----------已使用--------------
api.add_resource(RAGDocumentStatusApi,
