...
from controllers.service_api.dataset.error import DocumentIndexingError, InvalidMetadataError
...
class RAGQueryApi(DatasetApiResource):
    def post(self, tenant_id, ):
        ...
        parser.add_argument('reorgenaize', type=bool, default=True, required=True, nullable=False,
                            location='json')
        parser.add_argument('top_k', type=int, default=1, required=True, nullable=False,
                            location='json')
        parser.add_argument('score_threshold', type=float,default=0.7,required=False, nullable=True,
            location='json')
        parser.add_argument('query', type=str,  required=True, nullable=False,
                            location='json')
...
class DatasetQueryApi(DatasetApiResource):
    ...
    def get(self,tenant_id, dataset_id):
        ...
        response = {
            'data': marshal(dataset_queries, DatasetQueryApi.dataset_query_detail_fields_with_comment),
...
class RAGChunckListApi(DatasetApiResource):
    ...
    def get(self, tenant_id,doc_id,page_no,page_size):
        doc_id = str(doc_id)
        custom_dataset_retrieval = CustomDataSetRetrieval()
        return custom_dataset_retrieval.get_chuncks_by_order(doc_id=doc_id,page_number=page_no,page_size=page_size),200
...
api.add_resource(RAGDocumentDetailApi, '/rag/datasets/<uuid:dataset_id>/documents/<uuid:document_id>')
api.add_resource(RAGDatasetRelatedAppListApi, '/rag/datasets/<uuid:dataset_id>/related-apps')
api.add_resource(RAGDocumentRenameApi,
