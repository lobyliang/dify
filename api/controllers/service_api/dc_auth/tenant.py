...
class TenantManage(Resource):
    ...
    # 所有需要先添加供应商的模型厂家
    providers_with_key = ["wenxin", "openai","jina","chatglm","yi","zhipuai","spart","spark","tongyi","nvidia"]
    ...
    def setDefaultModle(tenant_id:str):
        if not TenantManage._initialized:
            ...
            TenantManage.llm_config["provider"]=current_app.config["DEFAULT_LLM_MODEL_PROVIDER"]
            TenantManage.llm_config["config_from"]='predefined-model' if TenantManage.llm_config["provider"] in TenantManage.providers_with_key else None
            ...
            TenantManage.llm_config["credentials"] = json.loads(current_app.config["DEFAULT_LLM_MODEL_CREDENTIALS"])
            ...
            if current_app.config["DEFAULT_LLM_MODEL_PROVIDER"] in TenantManage.providers_with_key:
                TenantManage.providers.append({"provider":current_app.config["DEFAULT_LLM_MODEL_PROVIDER"],
                                      "credentials":json.loads(current_app.config["DEFAULT_LLM_MODEL_CREDENTIALS"])})
                all_providers.append(current_app.config["DEFAULT_LLM_MODEL_PROVIDER"])

            TenantManage.embeddings_config = {}
            TenantManage.embeddings_config["model"]=current_app.config["DEFAULT_EMBEDDING_MODEL"]
            TenantManage.embeddings_config["model_type"]='text-embedding'
            TenantManage.embeddings_config["provider"]=current_app.config["DEFAULT_EMBEDDING_MODEL_PROVIDER"]
            TenantManage.embeddings_config["config_from"]='predefined-model' if TenantManage.embeddings_config["provider"] in TenantManage.providers_with_key else None
            ...
            TenantManage.embeddings_config["credentials"] = json.loads(current_app.config["DEFAULT_EMBEDDING_MODEL_CREDENTIALS"])
            ...
            if current_app.config["DEFAULT_LLM_MODEL_PROVIDER"] in TenantManage.providers_with_key and\
               current_app.config["DEFAULT_LLM_MODEL_PROVIDER"] not in all_providers:
                TenantManage.providers.append({"provider":current_app.config["DEFAULT_LLM_MODEL_PROVIDER"],
                                      "credentials":json.loads(current_app.config["DEFAULT_LLM_MODEL_CREDENTIALS"])})
                all_providers.append(current_app.config["DEFAULT_LLM_MODEL_PROVIDER"])
            ...
            TenantManage.reranking_config["model"]=current_app.config["DEFAULT_RERANKING_MODEL"]
            TenantManage.reranking_config["model_type"]='rerank'
            TenantManage.reranking_config["provider"]=current_app.config["DEFAULT_RERANKING_MODEL_PROVIDER"]
            TenantManage.reranking_config["config_from"]= 'predefined-model' if TenantManage.reranking_config["provider"] in TenantManage.providers_with_key else None
            ...
            TenantManage.reranking_config["credentials"] = json.loads(current_app.config["DEFAULT_RERANKING_MODEL_CREDENTIALS"])
    ...
    def post(self):
        ...
        parser.add_argument('user_name', type=str, required=True, nullable=False, location='json')
        parser.add_argument('password', type=str, required=False, nullable=True, location='json')
        parser.add_argument('token', type=str, required=False, nullable=True, location='json')
        ...
        if not email:
            email = args['user_name']+"@"+current_app.config['DEFAULT_DERAM_CLOUD_EMAIL_DOMAIN']

        try:
            old_account = db.session.query(Account).filter(Account.email==args['email']).first()
            if old_account:
            ...
            logging.error(f"{args['user_name']} create tenant {tenant.id} failed for {e}")
            TenantManage.clear_tenant(tenant.id)
            return {"msg":"Failed to create tenant","error":str(e)},500

        return {"tannt_id":tenant.id,"account_id":account.id,"email":account.email,"dataset_api_key":dataset_api_key,"status":tenant.status,"create_at":str(tenant.created_at)},200
    ...
    def clear_tenant(tenant_id:str):
        ...
        try:
            if tenant:
                ...
                # 删除所有默认模型
                default_models = db.session.query(TenantDefaultModel).filter(TenantDefaultModel.tenant_id == tenant_id).all()
                for default_model in default_models:
                    db.session.delete(default_model)

                # 删除所有模型
                models = db.session.query(ProviderModel).filter(ProviderModel.tenant_id == tenant_id).all()
                for model in models:
    ...
    # The role of the current user in the ta table must be admin or owner
        ...
        return TenantManage.create_api_key(tenant_id,TenantManage.data_set_token_prefix,TenantManage.data_set_resource_type)
    @staticmethod
    ...
    # The role of the current user in the ta table must be admin or owner
        ...
        return TenantManage.create_api_key(tenant_id,TenantManage.app_token_prefix,TenantManage.app_resource_type)
