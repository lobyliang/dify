...
class DefaultModelService:
    ...
    def add_provider(tenant_id: str, model_config: dict):
        ...
        try:
            ...
            logging.info(
                f"tenant:{tenant_id} add model provider {model_config['provider']}"
            )
            return True
        except CredentialsValidateFailedError as ex:
            logging.info(
                f"tenant:{tenant_id} add model provider failed!\nProrvider:{model_config['provider']}\n Error:{ex}"
            )
    ...
    def add_model(tenant_id: str, model_config: dict):
        ...
        if model_config.get("load_balancing", {}).get("enabled", False):
            ...
            model_load_balancing_service.update_load_balancing_configs(
                ...
                configs=model_config.get("load_balancing").get(
                    "configs"
                ),  # args['load_balancing']['configs']
            )
            ...
            if model_config["config_from"] != "predefined-model":
                ...
                try:
                    ...
                    logging.info(
                        f"tenant:{tenant_id} add model :{model_config['model']}:{model_config['model_type']} "
                    )
                    return True
                except CredentialsValidateFailedError as ex:
                    logging.warning(
                        f"tenant:{tenant_id}: add model failed!\nmodel:{model_config['model']}:{model_config['model_type']}\nreason:{str(ex)}"
                    )
                    # raise ValueError(str(ex))
    ...
    def setDefaultModel(tenant_id: str, model_config: dict):
        ...
        if "model_type" not in model_config or model_config["model_type"] not in [
        ...
        try:
            ...
            logging.info(
                f"tenant:{tenant_id}:model:{model_config['model']}:{model_config['model_type']} saved as default model"
            )
            return True
        except Exception:
