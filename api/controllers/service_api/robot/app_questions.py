...
class CreateAppQuestionApi(DatasetApiResource):
    ...
    def post(self, tenant_id, app_id):
        ...
        argparser.add_argument(
            "is_virtual", type=bool, required=False, default=False, location="json"
        )
        ...
        try:
            documents_ids = QuestionService.add_question(
                app_id, questions, tenant_id, is_virtual
            )
...
class GetAppQuestionApi(DatasetApiResource):
    ...
    def post(self, tenant_id: str):
        ...
        # argparser.add_argument('tenant_id', type=str, required=True,location='json')
        argparser.add_argument(
            "status", type=str, default=None, required=False, location="json"
        )
        argparser.add_argument(
            "page_no", type=int, default=None, required=False, location="json"
        )
        argparser.add_argument(
            "page_size", type=int, default=None, required=False, location="json"
        )
        args = argparser.parse_args()  # strict=True
        app_id = args["app_id"]
        # tenant_id = args['tenant_id']
        ...
        try:
            documents_ids, count = QuestionService.get_app_questions(
                tenant_id, app_id, status, page_no, page_size
            )
            # return Response(json.dumps([doc.to_dict() for doc in documents_ids]),status=200)
            return {
                "items": marshal(
                    documents_ids, AppQuestionField
                ),  # json.dumps([doc.to_dict() for doc in documents_ids]),
                "size": len(documents_ids),
                "page": page_no,
                "pageSize": page_size,
                "total": count,
                "hasMore": (
                    ((page_no - 1) * page_size + len(documents_ids)) < count
...
class MarchAppQuestionApi(DatasetApiResource):
    ...
    def post(self, tenant_id: str):
        ...
        return QuestionService.march_app_question(
            tenant_id,
            args["query"],
            args["top_k"],
