const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'Dream 賬號',
    langGeniusAccountTip: '您的 Dream 賬號和相關的使用者資料。',
    editName: '編輯名字',
  ...
  members: {
    ...
    invitationSentTip: '邀請已傳送，對方登入 Dream 後即可訪問你的團隊資料。',
    invitationLink: '邀請連結',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: '體驗',
      exhausted: '超出限額',
      desc: '託管 OpenAI 由 Dream 提供的託管 OpenAI 服務，你可以使用 GPT-3.5 等模型，在體驗額度消耗完畢前你需要設定其它模型供應商。',
      callTimes: '呼叫次數',
  ...
  apiBasedExtension: {
    title: 'API 擴充套件提供了一個集中式的 API 管理，在此統一新增 API 配置後，方便在 Dream 上的各類應用中直接使用。',
    link: '瞭解如何開發您自己的 API 擴充套件。',
    linkUrl: 'https://docs.dream.ai/v/zh-hans/advanced/api_based_extension',
    add: '新增 API 擴充套件',
    selector: {
      title: 'API 擴充套件',
  ...
  about: {
    changeLog: '更新日誌',
    updateNow: '現在更新',
    nowAvailable: 'Dream {{version}} 現已可用。',
    latestAvailable: 'Dream {{version}} 已是最新版本。',
  },
  appMenus: {
    overview: '概覽',
    promptEng: '編排',
