...
async function embedChatbot() {
  ...
  const baseUrl =
    difyChatbotConfig.baseUrl || `https://${isDev ? "dev." : ""}udify.app`;
  const openIcon = `<svg
            id="openIcon"
            width="24"
            height="24"
  ...
  function createIframe() {
    ...
    iframe.title = "dream chatbot bubble window";
    iframe.id = "dream-chatbot-bubble-window";
  ...
  const targetButton = document.getElementById("dream-chatbot-bubble-button");
  if (!targetButton) {
    // create button
    const containerDiv = document.createElement("div");
    containerDiv.id = "dream-chatbot-bubble-button";
