...
import ImageUpload from "@/app/components/base/image-upload";
...
import {
  ...
  fetchSegFile,
  deleteBindSegFile,
} from "@/service/v1/chunckAttach/files";
...
export const SegmentIndexTag: FC<{
  ...
  return (
    <div
      className={`text-gray-500 border border-gray-200 box-border flex items-center rounded-md italic text-[11px] pl-1 pr-1.5 font-medium ${className ?? ""}`}
    >
...
const SegmentDetailComponent: FC<ISegmentDetailProps> = ({
  ...
  const [fileUrl, setFileUrl] = useState(segInfo?.fileIds || "");
  const [fileCover, setFileCover] = useState(segInfo?.fileCovers || "");
  ...
  const handleCancel = () => {
    ...
    setFileUrl(segInfo?.fileIds || "");
    setFileCover(segInfo?.fileCovers || "");
  };

  const handleSave = () => {
    onUpdate(segInfo?.id || "", question, answer, keywords, fileUrl, fileCover);
  };

  const onRemoveFile = (file: any) => {
    deleteBindSegFile(segInfo?.id || "", {
      file_id: file.uid,
    });
  };
  ...
  const renderFileCover = () => {
    return (
      (fileCover || isEditing) && (
        <>
          <div className={s.keywordTitle}>封面</div>
          <div>
            <FileUpload
              fileUrl={fileCover}
  ...
  const renderFileContent = () => {
    return (
      (fileUrl || isEditing) && (
        <>
          <div className={s.keywordTitle}>附件</div>
          <div>
            <FileUpload
              fileUrl={fileUrl}
              onChange={(file: string) => {
  ...
  return (
    <div className={"flex flex-col relative"}>
      ...
      <SegmentIndexTag
        ...
        className="w-fit mt-[2px] mb-6"
      />
      <div className={s.segModalContent}>{renderContent()}</div>
      <div className={s.fileWrapper}>{renderFileCover()}</div>
      <div className={s.fileWrapper}>{renderFileContent()}</div>
...
const Completed: FC<ICompletedProps> = ({
  ...
  const onClickCard = async (detail: SegmentDetailModel) => {
    const data = await fetchSegFile(detail.id);
    detail.fileIds = data.attach.map((item) => item.id).join(",");
    detail.fileCovers = data.cover.map((item) => item.id).join(",");
