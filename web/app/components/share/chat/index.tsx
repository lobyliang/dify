...
const Main: FC<IMainProps> = ({ isInstalledApp = false, installedAppInfo }) => {
  ...
  useEffect(() => {
    if (siteInfo?.title) {
      ...
      else document.title = `${siteInfo.title} - Powered by Dream`;
    }
  ...
  const onMoreLoaded = useCallback(
    ({ data: conversations, has_more }: any) => {
      setHasMore(has_more);
  ...
  const onPinnedMoreLoaded = useCallback(
    ({ data: conversations, has_more }: any) => {
  ...
  const [citationConfig, setCitationConfig] = useState<CitationConfig | null>(
    null,
  );
  ...
  const fetchInitData = async () => {
    ...
    return Promise.all([
      ...
      fetchAppMeta(isInstalledApp, installedAppInfo?.id),
    ]);
  };

  const { data: fileUploadConfigResponse } = useSWR(
    isInstalledApp ? { url: "/files/upload" } : null,
  ...
  useEffect(() => {
    (async () => {
      try {
        const [appData, conversationData, appParams, appMeta]: any =
          await fetchInitData();
        setAppMeta(appMeta);
        const {
          app_id: appId,
  ...
  const handlePinnedListChanged = useCallback(
    (list: ConversationItem[]) => {
  ...
  return (
    <div className="bg-gray-100 h-full flex flex-col">
      ...
      >
        ...
        {/* main */}
        <div className={cn("h-full flex-grow flex flex-col overflow-y-auto")}>
          <ConfigSence
            ...
            >
              <div className="h-full overflow-y-auto" ref={chatListDomRef}>
                <Chat
                  ...
                  canStopResponding={
                    !!messageTaskId && isRespondingConIsCurrCon
                  }
                  ...
                  visionConfig={{
                    ...visionConfig,
                    image_file_size_limit: fileUploadConfigResponse
                      ? fileUploadConfigResponse.image_file_size_limit
                      : visionConfig.image_file_size_limit,
                  }}
